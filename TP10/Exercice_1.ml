...
let rec w n = if n <= 1 then 1 else list_w n
and list_w n = somme (List.map w (diviseurs_stricts n))
...
let premier n = List.length (diviseurs_stricts n) = 1
...
let b n = w (produit (List.map p (range n)))
let quelques_termes f combien = List.map f (range combien)
...
let rec get_var s l =
  ...
  | x :: q ->
      let a, n = x in
      if a = s then n else get_var s q
;;
  ...
  let tmp =
    Mult
      ( Add (Int 4, Int 9),
        IfTE
          ( Or
              ( Equal (Add (Int 1, Int 1), Int 3),
                And (Equal (Int 4, Variable "foo"), Equal (Int 2, Int 2)) ),
            Variable "bar",
            Int 86 ) )
  in
  assert (eval_arith tmp [ ("foo", 4); ("bar", -1) ] = -13);
